...
def _preprocess_with_masks(source_csv: str) -> tuple:
    ...
    if is_clean:
        ...
        X, y, feature_cols, null_mask = _encode(df_all)
    ...
    for i in range(n_show):
        ...
        for d in range(len(vec)):
            ...
            if mask[d]:
                ...
                status = "● observed"
                role   = "→ (Δ)² + 1 (squared diff + presence weight)"
...
def _print_neighbor_distances(X, null_mask, edge_index, feature_cols=None,
    ...
    print(f"{'═'*95}")
    print(f"  Rules:  both_observed → (x_i - x_j)² + 1    one_null → 0.0    both_null → SKIP")
    print(f"{'═'*95}")
...
def _compute_pair_distance(X, null_mask, i, j, d_total):
    ...
    Distance per dimension:
        both_observed → (x_i - x_j)² + 1   (squared diff + presence weight)
    ...
    for dim in range(d_total):
        ...
        if null_i and null_j:
            ...
            sq = (v_i - v_j) ** 2
            contrib = sq + 1.0     # presence weight: always ≥ 1
...
def _build_knn_graph(X: np.ndarray, null_mask: np.ndarray,
    ...
    The +1 presence weight ensures that observed-vs-observed comparisons
    always contribute positively, preventing distance=0 ties between
    nodes that merely share identical values on a few surviving dimensions.
    ...
    if has_null:
        ...
        print(f"\n[graph_builder]   Distance rule: both_observed→(Δ)²+1, one_null→0, both_null→skip")
...
def _null_aware_knn(X: np.ndarray, null_mask: np.ndarray, k: int) -> torch.Tensor:
    ...
    For each pair (i, j), the distance across d dimensions is:
        ...
        For each dimension d:
          - both_observed (null_mask[i,d]=F AND null_mask[j,d]=F):
                → (X[i,d] - X[j,d])² + 1   (squared diff PLUS presence weight)
    ...
    WHY +1?  Without it, two nodes that happen to share the same observed
    values get distance=0, which is indistinguishable from two nodes that
    are only connected through null dimensions (one_null→0).  The +1
    ensures that every observed-vs-observed comparison contributes a
    positive baseline, so distance reflects HOW MANY real dimensions
    were compared, not just whether the compared values differ.

    Effect:
      • Nodes with many shared observed dims → higher distance (more
        dimensions contributing ≥1 each) → properly discriminated
      • Nodes with mostly null dims → lower distance (fewer contributing
        dims) → recognised as "can't really compare these well"
      • Identical observed values → contribute exactly 1.0 (not 0)
      • Different observed values → contribute >1.0 (1 + squared diff)
    """
    n, d = X.shape
    ...
    for start in range(0, n, chunk):
        ...
        # Squared difference + 1 where both_observed, 0 elsewhere
        diff = Xi[:, np.newaxis, :] - X[np.newaxis, :, :]
        contrib = np.where(both_observed, diff ** 2 + 1.0, 0.0)
