...
Configuration (in config.py):
    ...
    (random state is taken from config.RANDOM_SEED, set per seed by
     run_batch.py's outer seed loop)
"""
...
def impute_mice(df: pd.DataFrame, target_col: str,
    ...
    # Read the current seed at CALL time so that multi-seed batch runs
    # (config.set_seed(s) in run_batch.py) actually propagate into MICE.
    # Previously this used a separate MICE_RANDOM_STATE constant (=42),
    # which meant MICE produced identical output for every seed.
